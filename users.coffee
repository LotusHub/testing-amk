...
class SteamAccount
  ...
  onError: (e) =>
    if e.eresult == Steam.EResult.InvalidPassword
      ...
      console.log(chalk.bold.red("X ") + chalk.white("Logon failed for account '#{@accountName}' - already logged in somewhere"))
    else if e.eresult == Steam.EResult.AccountLogonDenied
      query = {type: 'input', name: 'steamguard', message: 'enter steamguard code: '}
...
query = [
  ...
  {type: 'checkbox', name: 'u_games', message: 'game selection ', choices: POSSIBLE_GAMES}
]
